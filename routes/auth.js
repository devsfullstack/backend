...
router.get('/usuario-logueado', (req, res) => {
    try {
        ...
        const userId = decoded.id; // Suponiendo que el token tiene el campo u1_id que representa el ID del usuario
  
        // Consultar la base de datos para obtener el nombre de usuario
        const [rows] = pool.query(`SELECT usuario FROM usuarios WHERE u1_id = ?`, [userId]);
